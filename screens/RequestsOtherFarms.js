...
export default function RequestsOtherFarms({ navigation }) {
  ...
  const { otherRequests, LoadOtherRequests, AcceptRequest, RejectRequest, sorters, maxPage } = useContext(MyRequestsOtherFarmsContext);
      ...
      <>
        <View style={tw`items-center mb-1 px-2`}>
          ...
          <View style={[tw`w-full  h-42px px-7 items-center flex-row border-b-2 justify-center`, { borderColor: "rgba(81, 212, 0, 0.2)" }]}>
            ...
            <TouchableOpacity style={[tw`items-center flex-row h-31px px-3 rounded-xl ml-3`, { backgroundColor: "rgba(239, 68, 68, 1)" }]} onPress={() => RejectRequest(idRequest)}><Text style={tw`text-white text-center grow font-bold uppercase text-12px mr-1`}>Rechazar</Text><Feather name="x-circle" size={22} color="white" /></TouchableOpacity>
          </View>
        </View>
