    ...
    <SafeAreaProvider>
      <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView>
          <View style={styles.container}>
            <View style={tw`flex-row justify-between`}>
              ...
              <View style={tw`items-end`}>
                <SearchComponent
                  GetElements={FindCoffeeBush}
                  GetOriginalElements={GetCoffeeBushs}
                  secondParameter={global.idCrop}
                />
                <AddButton
                  onPress={() =>
                    setIsModalOpenAddCoffeeBush(!isModalOpenAddCoffeeBush)
                  }
                />
              </View>
            </View>
          ...
          <View
            style={[tw`flex flex-wrap flex-row justify-center items-center`]}
          >
            {coffeeBushs.length > 0 ? (
              coffeeBushs.map((coffeeBush, index) => {
                return (
                  <CardBush
                    qrCode={coffeeBush.qrCode}
                    id={coffeeBush.idCoffeeBush}
                    date={coffeeBush.createdDdate}
                    key={index}
                  >
                    <CardCoffeeButton
                      onPress={() => {
                    ...
                    <CardCoffeeButton
                      onPress={() => {
                        (global.idToDelete = coffeeBush.idCoffeeBush),
