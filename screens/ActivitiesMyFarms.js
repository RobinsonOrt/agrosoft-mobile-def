...
const ActivitiesMyFarms = ({ navigation }) => {
  const [isModalOpenAddActivityMyFarms, setIsModalOpenAddActivityMyFarms] =
    useState(false);
  const [
    isModalOpenModifyActivityMyFarms,
    setIsModalOpenModifyActivityMyFarms,
  ] = useState(false);
  ...
  const subHeaderTable = (
    <View
      style={[
        tw`w-full  items-center flex-row h-8`,
      ...
      {activities.length > 0 ? (
        activities.map((activity, index) => {
          return (
            <View
              style={[
                tw`w-full items-center flex-row  pt-2 border-t`,
                { borderColor: "rgba(81, 212, 0, 0.2)" },
              ]}
              key={index}
            >
              <View style={tw`w-35% px-2`}>
                <Text style={tw`text-justify w-full`}>
              ...
              <View style={tw`w-23% items-center`}>
                <View style={tw`my-1`}>
                  <CartButtonCrop
                    text={"Campos  "}
                    onPress={() => {
                      (global.idActivity = activity.idActivity),
                        navigation.navigate("Fields");
                    }}
                    color={"rgba(34, 158, 197, 1)"}
                    icon={<AntDesign name="search1" size={12} color="white" />}
                  />
                </View>
                <View style={tw`my-1`}>
                  <CartButtonCrop
                    ...
                    icon={<AntDesign name="edit" size={15} color="white" />}
                  />
                </View>
                <View style={tw`my-1`}>
                  <CartButtonCrop
                    text={"Eliminar"}
                    onPress={() => {
                      (global.idToDelete = activity.idActivity),
                        (global.idActivityType = activityType),
  ...
  return (
    <>
      <View style={tw`h-full w-full px-0`}>
        ...
        <ModalAddActivityMyFarms
          isModalOpenAddActivityMyFarms={isModalOpenAddActivityMyFarms}
          setIsModalOpenAddActivityMyFarms={setIsModalOpenAddActivityMyFarms}
        />
        <ModalModifyActivityMyFarms
          isModalOpenModifyActivityMyFarms={isModalOpenModifyActivityMyFarms}
          setIsModalOpenModifyActivityMyFarms={
            setIsModalOpenModifyActivityMyFarms
          }
        />
          ...
          <View style={styles.container}>
            <View style={tw`flex-row justify-between mb-5`}>
              <View style={tw``}>
                <SorterComponent
                  sorters={sorters}
                  sorter={"name_activity"}
                  GetElements={GetActivities}
                  firstParameter={activityType}
                  secondParameter={farm.idFarm}
                />
              </View>
              <View style={tw`items-end`}>
                <SearchComponent
                  GetElements={FindActivities}
                  GetOriginalElements={GetActivities}
                  secondParameter={activityType}
                  thirdParameter={farm.idFarm}
                />
                <AddButton
                  onPress={() => {
                    (global.idActivityType = activityType),
                      setIsModalOpenAddActivityMyFarms(
                        !isModalOpenAddActivityMyFarms
                      );
                  }}
                />
              </View>
