      ...
      <SafeAreaProvider>
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.container}>
            ...
            <ModalChangeLabor isModalOpenChangeLabor={isModalOpenChangeLabor} setIsModalOpenChangeLabor={setIsModalOpenChangeLabor} />
            <ModalAddLabor isModalOpenAddLabor={isModalOpenAddLabor} setIsModalOpenAddLabor={setIsModalOpenAddLabor} />
            <ModalEmployeeDelete isModalOpenEmployeeDelete={isModalOpenEmployeeDelete} setIsModalOpenEmployeeDelete={setIsModalOpenEmployeeDelete} />
            {(employees.length > 0) ? (
                ...
                <Accordion title={employee.name} key={index}>
                  <View style={tw`bg-gray-300 w-full p-4 pt-3 mt-1 rounded-xl`}>
                    ...
                    <TextInput
                      ...
                      editable={false}
                    />
                    <TouchableOpacity style={tw`bg-yellow-400 justify-between items-center flex-row h-52px p-4 mt-3 mb-2 rounded-xl`} onPress={() => {
                      global.idEmployee = employee.idUser;
                      setIsModalOpenChangeLabor(!isModalOpenChangeLabor)
                    }}><Text style={tw`text-white text-center grow font-bold`}>Editar Cargo</Text><Image source={Edit} style={tw`h-24px w-24px float-right`} /></TouchableOpacity>
                    <TouchableOpacity style={tw`bg-yellow-500 justify-between items-center flex-row w-full h-52px p-4 mt-3 rounded-xl`} ><Text style={tw`text-white text-center grow font-bold`}>Ver Cultivos Asignados</Text><Image source={Enter} style={tw`h-24px w-24px float-right`} /></TouchableOpacity>
                    <TouchableOpacity style={tw`bg-red-500 justify-between items-center flex-row h-52px p-4 mt-3 rounded-xl`} onPress={() => {
                      global.idEmployee = employee.idUser;
                      setIsModalOpenEmployeeDelete(!isModalOpenEmployeeDelete)
                    }}><Text style={tw`text-white text-center grow font-bold`}>Eliminar Empleado</Text><Image source={Leave} style={tw`h-24px w-24px float-right`} /></TouchableOpacity>
                  </View>
                </Accordion>
              )
            })) : (<View style={styles.container}>
