...
const Employee = ({ navigation }) => {
  ...
  const { LoadEmployees, employees, sorters, maxPage } = useContext(MyEmployeesContext);
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.container}>
          <View style={tw`flex-row justify-between`}>
            <View style={tw``}>
              <SorterComponent sorters={sorters} sorter={"name"} GetElements={LoadEmployees} firstParameter={global.idFarm} />
            </View>
          ...
          {employees.length > 0 ? (
            employees.map((employee, index) => {
              return (
                <AccordionEmployees name={employee.name + " " + employee.lastName} email={employee.email} key={index} >
                  <View style={[tw`w-full`]}>
                    <FieldsItems tittle={"Cargo:"} value={employee.nameSubRole} onPress={()=>{console.log("hdhd")}} color={"rgba(234, 179, 8, 1)"} text={"Editar"} icon={<Feather name="edit-3" size={18} color="white" />} />
                    ...
                    <FieldsItems tittle={"Opciones:"} onPress={()=>{console.log("hdhd")}} color={"rgba(239, 68, 68, 1)"} text={"Eliminar"} icon={<AntDesign name="delete" size={18} color="white" />} />
                  </View>
                </AccordionEmployees>
              );
            })
