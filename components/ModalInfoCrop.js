...
const ModalInfoCrop = ({isModalOpenInfoCrop, setIsModalOpenInfoCrop}) => {
    ...
    return(
        <ModalModel isModalOpen={isModalOpenInfoCrop} setIsModalOpen={setIsModalOpenInfoCrop}>
            ...
            <View style={[tw`h-50px items-center w-full flex-row rounded-t-15px`, { backgroundColor: "rgba(32, 84, 0, 0.1)" }]}><Text style={tw`text-center grow text-black text-18px font-bold uppercase`}>{crop.nameCrop}</Text></View>
                
                    <Text style={tw`text-15px font-bold text-center mb-3 mt-4`}>Descripción: </Text>
                    <View style={tw`w-full items-center flex-row justify-center mb-2`}><Text style={tw`text-14px w-210px text-justify`}>{crop.descriptionCrop}</Text></View>                
                ...
                <View style={[tw`items-center flex-row w-full h-30px border rounded-7px px-2 mt-2`, {backgroundColor:"rgba(32, 84, 0, 0.15)", borderColor:"rgba(156, 163, 175, 1)"}]}>
                    ...
                    <Text style={tw`text-12px text-center mr-2`}>{coffeeBushCount}</Text>
                </View>
                <View style={[tw`items-center flex-row w-full h-30px border rounded-7px px-2 mt-2`, {backgroundColor:"rgba(32, 84, 0, 0.15)", borderColor:"rgba(156, 163, 175, 1)"}]}>
                    <Text style={tw`text-12px font-bold text-center mr-2`}>Variedad del Café:</Text>
                    <Text style={tw`text-12px text-center mr-2`}>{crop.coffeeVariety}</Text>
                </View>
            ...
            <View style={tw`mt-7 mx-8`}>
                    ...
                    <ModalButton text={"Cerrar"} onPress={() => {setIsModalOpenInfoCrop(!isModalOpenInfoCrop)}} color={"rgba(239, 68, 68, 1)"}/>
            </View>
