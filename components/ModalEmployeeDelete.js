...
export default function ModalEmployeeDelete({ isModalOpenEmployeeDelete, setIsModalOpenEmployeeDelete }) {
  const { DeleteEmployee } = useContext(MyEmployeesContext);
  ...
  const onSubmit = async () => {
    ...
    DeleteEmployee(data);
    setIsModalOpenEmployeeDelete(false);
  }
  ...
  return (
    <>
      <Modal visible={isModalOpenEmployeeDelete} transparent={true} animationType={'fade'}>
            ...
            <View style={tw`h-full flex items-center mt-5 justify-center`}>
              ...
              <View style={tw`px-7 mb-10 flex items-center justify-center`}>
                <Text style={tw` text-black mb-10 w-283px  text-center`}>
                  ¿Está seguro que quiere eliminar al empleado Nombre con correo?
                </Text>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => setIsModalOpenEmployeeDelete(!setIsModalOpenEmployeeDelete)}
                >
